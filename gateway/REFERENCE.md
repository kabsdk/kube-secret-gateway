...
This document defines the gateway's complete API and operational behavior.
Start with the [gateway guide](README.md) for the shorter introduction and
deployment example.
...
| --- | --- | --- |
| Secrets | `:8080` | `/secrets/{exposure}/{key}`, `/bundles/{exposure}` |
| Operations | `:8081` | `/healthz`, `/readyz`, `/metrics` |

Only the Secrets listener should be externally routed. Both secret endpoints
accept `GET` and `HEAD`; `HEAD` returns the same status and headers without a
body. Query strings are ignored.

Paths must match exactly. Extra or missing segments, trailing slashes, `.` or
`..`, and any percent-encoding return `404`; the gateway never redirects.
There is no listing endpoint. Clients must already know the exposure name.

### One value

`GET /secrets/{exposure}/{key}` returns the exact bytes stored under one
Secret key:
...
Use this route for an independent value. Fetch related values as a bundle so
an upstream update cannot occur between requests.

### Bundle

`GET /bundles/{exposure}` returns every key served by the exposure from one
read of one Secret version:
...
An empty exposure returns `{}`. If an `includeKeys` entry is missing, the
whole request returns `503`; bundles are never partial.

### ETags and polling

Every successful response has an opaque ETag covering the returned bytes. A
bundle tag changes when any exposed key is added, removed, or changed.
Recreating a Secret also changes its tags because its UID changes. Tags are
identical across gateway replicas.
...
Each exposure uses HTTP Basic Auth. The gateway evaluates requests in this
order:
...
| 8 | A required `includeKeys` key is missing | `503` | `expected_key_missing` |
| 8 | A requested key is not exposed | `404` | `key_not_found` |
...
- Without valid credentials, existing, excluded, and missing keys all return
  `401`.
...
1. Fetch related values through `/bundles/`.
...
5. Store the new ETag last, after files and any reload command succeed, so a
   crash or reload failure causes the complete bundle to be installed again.
...
The gateway reads `/etc/kube-secret-gateway/config.yaml`, or the path supplied
...
| --- | --- | --- |
| `server.listenAddress` | `:8080` | Secrets listener. |
...
| `secrets` | required | One or more exposures. |

See [`examples/config.yaml`](examples/config.yaml) for a complete document.

### Exposures

Each `secrets` entry has these fields:

| Field | Required | Meaning |
| --- | --- | --- |
| `name` | no | URL name; defaults to `secretRef.name` and must be unique. |
...
| `auth.type` | yes | Currently only `basicAuth`. |
| `auth.secretRef` | yes | Secret containing the exposure credentials. |
| `includeKeys` | no | Serve only these keys and require all of them to exist. |
| `excludeKeys` | no | Serve every key except these. |

`includeKeys` and `excludeKeys` are mutually exclusive. With neither, all
current and future keys are exposed. A filtered key behaves exactly like a key
that does not exist. Exposure names follow Kubernetes name syntax: lowercase
letters, digits, `-`, and `.`.

A reference without an explicit namespace uses `defaultNamespace`. If neither
is set, validation fails. The gateway never assumes its own namespace and
...
are stored there by Kubernetes as well. A cert-manager TLS Secret commonly has
`tls.crt`, `tls.key`, and sometimes `ca.crt`.

An authentication Secret must contain non-empty `username` and `password`
keys; other keys are ignored. Values are compared byte-for-byte, so a trailing
newline is part of the password. Credentials are checked from watched state on
every request and can be rotated without restarting the gateway. Comparison
uses constant-time SHA-256 digests.
...
credential keys will be exposed unless `includeKeys` or `excludeKeys` prevents
that. Separate Secrets are safer and clearer.

## Kubernetes synchronization

Every distinct source or authentication Secret is managed once, even when
several exposures reference it. The gateway performs an exact-name list, then
...
runtime state: the gateway continues watching and serves it once created. If
...
- Route both `/secrets/` and `/bundles/` on the HTTP router.
...
could preserve it with the PROXY protocol, but the gateway does not implement
...
The gateway has no `caFile` because client certificates are not supported.
...
and the gateway reloads the mounted pair automatically.
...
The gateway needs `get`, `list`, and `watch` on every source, exposure-auth,
...
The gateway never lists every Secret in a namespace.

Create one Role per referenced namespace and bind it to the gateway
ServiceAccount. [`examples/kubernetes.yaml`](examples/kubernetes.yaml) shows
...
Logs are structured. Requests include exposure, key, resolved client address,
status, reason, and duration, but never credentials or values.

At startup the gateway validates configuration, loads TLS certificates,
...
- Allow-list and authentication ordering prevents exposure and key probing.
...
- The gateway does not support the PROXY protocol.
...
- A bundle covers one Kubernetes Secret; different Secrets have no shared
  version and cannot form one consistent bundle.
